...
public class sfaB2BLead extends SFAMenu {
    ...
    public sfaB2BLead(AutomationTool tool, Test test, User user) throws Exception {
        super(tool, test, user);

        if (!tool.checkDisplayedUsingXpath("//div[contains(text(), 'B2B Lead')]")) {
            throw new IllegalStateException("Expected B2B Lead screen doesn't show");
        }
    }

    void clickNewB2BLead() throws Exception {
        tool.clickUsingID("Awrp1y3_label");
        Common.sleepForNumberOfSeconds(3);
    }

    void setB2BLeadName(String b2bLeadName) throws Exception {

        tool.enterStringUsingId("Ix18yor", b2bLeadName);
    ...
    public void findAndOpenB2BLeadByName(String b2bLeadName) throws Exception {
        ...
        tool.enterStringUsingId("Iaw2djm", b2bLeadName);
        tool.clickUsingID("$2mtu0_filterbutton"); // Click
        // Search
        Common.sleepForNumberOfSeconds(2);
        tool.clickUsingID("$2mtu0_cell_0_0_Img"); // Open
        // B2B
        // Lead
        Common.sleepForNumberOfSeconds(3);
    }
    ...
    public String getPriceList() throws Exception {
        return tool.getSelectedTextByID("I8ea3x9");
    }

    public String getFirstProductName() throws Exception {
        return tool.getTextUsingId("$5rwlaq_cell_0_1_span");
    }
