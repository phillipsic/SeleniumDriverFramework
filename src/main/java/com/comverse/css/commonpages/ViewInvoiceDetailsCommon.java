...
public class ViewInvoiceDetailsCommon extends CommonMenu {
    ...
    public InvoiceImageInTextCommon clickAsciiImage() throws Exception {  test.writeInLog(Common.getMethodName());
        tool.clickUsingXPath("//img[@title='Simple format - Ascii']");
        return new InvoiceImageInTextCommon(tool, test, user);
    }

    public ViewInvoiceBalanceDetailsCommon clickBalanceName() throws Exception {  test.writeInLog(Common.getMethodName());
    ...
    public BilledAdjustmentsCommon clickBilledAdjustments() throws Exception {  test.writeInLog(Common.getMethodName());
        tool.clickUsingID("youcan_VIEW_ADJUSTMENTS");
        return new BilledAdjustmentsCommon(tool, test, user);
    }

    public ViewPaymentDistributionsCommon clickPaymentDistribution() throws Exception {  test.writeInLog(Common.getMethodName());
        tool.clickUsingID("youcan_VIEW_PAYMENT_DISTRIBUTIONS");
        return new ViewPaymentDistributionsCommon(tool, test, user);
    }

    public ViewBilledRCNRCTermsCommon clickBilledRCNRC() throws Exception {  test.writeInLog(Common.getMethodName());
        tool.clickUsingID("youcan_VIEW_RC_NRC");
        return new ViewBilledRCNRCTermsCommon(tool, test, user);
    }

    public ViewBilledRechargesCommon clickBilledRecharges() throws Exception {  test.writeInLog(Common.getMethodName());
        tool.clickUsingID("youcan_VIEW_INVOICE_RECHARGES");
        return new ViewBilledRechargesCommon(tool, test, user);
    }

    public ViewBilledUsageCommon clickBilledUsages() throws Exception {  test.writeInLog(Common.getMethodName());
        tool.clickUsingID("youcan_VIEW_USAGES");
        return new ViewBilledUsageCommon(tool, test, user);
    }

    public ViewBilledMTRCommon clickBilledMTR() throws Exception {  test.writeInLog(Common.getMethodName());
        tool.clickUsingID("youcan_VIEW_INVOICE_MTR");
        return new ViewBilledMTRCommon(tool, test, user);
    }

    public ViewInvoicesCommon clickBack() throws Exception {  test.writeInLog(Common.getMethodName());
